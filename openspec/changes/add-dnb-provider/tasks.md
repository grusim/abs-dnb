...
- [ ] Create `pyproject.toml` with `[project]` metadata (name `abs-dnb`,
      version `0.1.0`, Python ≥ 3.12, MIT license) and dependencies:
      `fastapi`, `uvicorn[standard]`, `httpx`, `pymarc`.
- [ ] Add `[dependency-groups] dev = ["pytest", "pytest-asyncio", "httpx",
      "respx"]`.
- [ ] Run `uv sync` to generate `uv.lock`.
- [ ] Confirm `uv run uvicorn abs_dnb.main:app --reload` starts on port 8000.
...
- [ ] Write `tests/test_marc_parser.py` — unit tests for the MARC21 parser:
...
- [ ] Implement `abs_dnb/marc.py` until `pytest tests/test_marc_parser.py`
      passes.
- [ ] Write `tests/test_search.py` — integration tests mocking DNB SRU:
...
- [ ] Implement `abs_dnb/dnb.py` (SRU client) until tests pass.
- [ ] Write `tests/test_covers.py` — unit tests for cover chain:
...
- [ ] Implement `abs_dnb/covers.py` until tests pass.
- [ ] Wire everything into `abs_dnb/main.py` (FastAPI app, `/search` route,
      optional health `GET /`).
- [ ] Evaluate CQL strategy: test `WOE=<words>` vs `TIT=<title> AND VER=<author>`
      against fixtures; document chosen strategy in a comment in `dnb.py`.
- [ ] Decide on `language` filter default (opt-in vs always-`ger`); add
      `?language=ger` query param if opt-in chosen.
- [ ] `uv run pytest` — all tests green.

## 3. Dockerfile

- [ ] Write `Dockerfile` (single-stage, `python:3.12-slim`):
  - Install `uv`, copy `pyproject.toml` + `uv.lock`, run `uv sync --no-dev`.
  - Copy `abs_dnb/` source.
  - `CMD ["uv", "run", "uvicorn", "abs_dnb.main:app", "--host", "0.0.0.0",
    "--port", "8000"]`.
  - Non-root user (`appuser`).
- [ ] `docker build -t abs-dnb:local .` — builds cleanly.
- [ ] `docker run --rm -p 8000:8000 abs-dnb:local` + manual
...
- [ ] Push `v0.1.0` tag — confirm Actions run completes and image appears on
      Docker Hub.

## 5. Verify published image

- [ ] `docker pull grusim/abs-dnb:0.1.0` on a clean machine (or CI artifact).
- [ ] `docker run --rm -p 8000:8000 grusim/abs-dnb:0.1.0` starts without error.
- [ ] `curl "http://localhost:8000/search?query=Leises+Gift+Iles"` → 1 match,
      `title` populated, `narrator` present, `cover` is a valid JPEG URL.
- [ ] `cosign verify docker.io/grusim/abs-dnb:0.1.0 --certificate-identity-regexp
      'github.com/grusim/abs-dnb' --certificate-oidc-issuer
      'https://token.actions.githubusercontent.com'` → verification OK.
- [ ] `docker sbom grusim/abs-dnb:0.1.0` (or `docker buildx imagetools inspect`)
